...
- **➡️ PRÓXIMA (retomar aqui):**
  - **Bloco 5 — poderes de classe** (impr. 54-77 = PDF 56-79): layout denso 2 colunas; seções por classe na ordem Arcanista→Paladino. **ATENÇÃO (achado):** os títulos de classe são estilizados e NÃO saem limpos no `pdftotext` (o grep por cabeçalho isolado só pegou Bardo p.57 e Paladino p.76). → **mapear as seções de classe POR VISÃO** (renderizar as páginas 300 DPI e identificar onde cada classe começa) antes de extrair. Extrair 1 `poder` por poder, `grupo`=classe, em `data/herois-de-arton/poderes/`. Subgrupos vistos: "Poderes de Brado" (Bárbaro), "Poderes de Paixão" (?), "Novos Efeitos de Golpe Pessoal" (Guerreiro).
  - **Bloco 6 — poderes gerais** (impr. 78-95 = PDF 80-97): grupos Combate/Destino/Magia/Tormenta/Raça/Grupo.
  - **Bloco 7 — tabelas de personagem** (impr. 96-101): tipo `regra`/`regra-de-criacao` com tabelas (nomes/trejeitos).
  - **Lote de arte das 13 variantes** (cada variante tem figura na sua página de abertura, PDF 26-47; extrair par cor+smask e compor).

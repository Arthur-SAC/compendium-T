...
- **➡️ PRÓXIMA (retomar aqui):**
  - **Bloco 5 — poderes de classe** (impr. 54-77 = PDF 56-79): layout denso 2 colunas; seções por classe na ordem Arcanista→Paladino (cabeçalhos isolados; vistos: Inventor PDF72, Lutador PDF75, Nobre PDF77; subgrupos "Poderes de Brado", "Poderes de Paixão"). Extrair 1 `poder` por poder, `grupo`=classe, em `data/herois-de-arton/poderes/`. **Mapear as seções de classe com cuidado** antes de atribuir grupo.

...
- **➡️ PRÓXIMA (retomar aqui):**
  ...
  - **➡️ Bloco 6 — poderes gerais** (impr. 78-95 = PDF 80-97). Mapa: **Combate** (Tabela 1-18, PDF ~80-83) · **Destino** (Tabela 1-19) · **Magia** (Tabela 1-20) · **Tormenta** (Tabela 1-21, PDF ~85) · **Raça** (Tabela 1-22, PDF 86-93 — poderes por raça) · **Grupo** (Tabela 1-23, PDF 94-97). Usar `grupo`=combate/destino/magia/tormenta/raca/grupo (minúsculo, p/ juntar aos do Básico no índice `/poderes`). As Tabelas 1-18..1-23 listam os poderes de cada grupo (índice) + descrições no corpo. Mesma técnica: pdftotext + visão p/ os títulos de grupo estilizados. CUIDADO: grupos Combate/Destino/Magia/Tormenta ficam intercalados em 2 colunas nas PDF 80-86.
  - **Bloco 7 — tabelas de personagem** (impr. 96-101 = PDF 98-103): tipo `regra`/`regra-de-criacao` com tabelas (nomes/trejeitos por povo).
  - **Lote de arte das 13 variantes** (cada variante tem figura na sua página de abertura, PDF 26-47; extrair par cor+smask e compor → `site/public/classes/<slug>.png` + `imagens:[...]`). Já têm arte: Alquimista, Treinador, 5 raças.
  - **Ao FECHAR a Onda 1:** rodar tsc+test+build, atualizar este PROGRESSO, e avisar o usuário (ele pedic "avisar quando terminar a Onda 1 inteira"). Depois: **Ondas 2 (Distinções, precisa código `distincao`), 3 (Arsenal), 4 (Regras Opcionais)**.

...
**Status (2026-06-10): 🔄 ATLAS DE ARTON — EM EXTRAÇÃO POR FASES.** Fonte `atlas-de-arton` (ordem 8). 484 págs, offset PDF=impressa+2. Decisão do usuário: regiões já existentes no Básico viram `regiao-expansao` (agregadas via lookup, NÃO sobrescrevem); regiões novas viram `regiao`. Conduzir por fases, autônomo. Dumps por página em `extracao/cache/atlas/pages/pdfNNN_impMMM.txt`.
- **ROADMAP (✅=feito):** ✅Onda A código · ✅Cap.1 O Reinado · ✅Cap.2 Além do Reinado · ✅Cap.3 Além de Arton · ⬜Intro (História/Calendário/Linha do Tempo) · ⬜Cap.4 Organizações (Exércitos&Mercenários, Ordens&Igrejas, Guildas&Irmandades → `regra`) · ⬜Cap.5 Vida em Arton (Sociedade, Viagens, Dinheiro, Arte, Lei, Vida de Aventureiro → `regra`) · ⬜Apêndice Origens Regionais (→ `origem`).
  - **Regiões: 39 no total (24 `regiao-expansao` + 15 `regiao` novas).** Cap.3 (4 exp: khubar, tres-mares, doherimm, moregnia[=Moreania] + 7 novas: lamnor, khalifor, galrasia, ubani, deserto-da-perdicao, tamu-ra, eter-divino).
  - **Próxima fatia (retomar aqui):** Intro + Cap.4 + Cap.5 + Apêndice (NÃO são regiões — são lore/regras/origens). Impressas: Breve História 12, Tempo & Calendário 30, Linha do Tempo 34 (já existe `linha-do-tempo` no Básico → avaliar expansão ou regra); Cap.4 Organizações 422–445 (Exércitos&Mercenários 424, Ordens&Igrejas 431, Guildas&Irmandades 439 → `regra`); Cap.5 Vida em Arton 446–469 (Sociedade 448, Viagens 450, Dinheiro 453, Arte 456, Lei 465, Vida de Aventureiro 468 → `regra`); Apêndice Origens Regionais 470+ (→ `origem`). Offset PDF=impressa+2. Dumps em `extracao/cache/atlas/pages/`.

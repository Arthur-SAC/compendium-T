...
- **ROADMAP (✅=feito):** ✅Onda A código · ✅Cap.1 O Reinado · ⬜Intro (História/Calendário/Linha do Tempo) · ⬜Cap.2 Além do Reinado (~18 regiões: Supremacia Purista, Conflagração do Aço, Aslothia, Repúblicas Livres, Salistick, Svalas, Feudos de Trebuck, Triunphus, Sckharshantallas, Sanguinárias, Uivantes, Ermos Púrpuras, Floresta de Tollon, Império de Tauron, Nova Malpetrim, Smokestone, Ruínas de Tyrondir, Catacumbas de Leverick) · ⬜Cap.3 Além de Arton (~11: Lamnor, Khalifor, Khubar, Galrasia, Três Mares, Ubani, Deserto da Perdição, Tamu-ra, Moreania, Doherimm, Éter Divino) · ⬜Cap.4 Organizações (Exércitos&Mercenários, Ordens&Igrejas, Guildas&Irmandades → `regra`) · ⬜Cap.5 Vida em Arton (Sociedade, Viagens, Dinheiro, Arte, Lei, Vida de Aventureiro → `regra`) · ⬜Apêndice Origens Regionais (→ `origem`).
